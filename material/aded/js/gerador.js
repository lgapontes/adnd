...
function render() {
  forcar_havenloft = document.getElementById('forcar_havenloft').checked;
  forcar_classe = document.getElementById('classes').options[document.getElementById('classes').selectedIndex].value;
  forcar_raca = document.getElementById('racas').options[document.getElementById('racas').selectedIndex].value;
...
function obter_valores_setados_atributos(id,atributo) {
  ...
  let valor = campo[campo.selectedIndex].value;
...
function obter_todos_atributos() {
  let retorno = {};
  retorno['forca'] = obter_valores_setados_atributos('forca','Força');
  retorno['destreza'] = obter_valores_setados_atributos('destreza','Destreza');
  retorno['constituicao'] = obter_valores_setados_atributos('constituicao','Constituição');
  retorno['inteligencia'] = obter_valores_setados_atributos('inteligencia','Inteligência');
  retorno['sabedoria'] = obter_valores_setados_atributos('sabedoria','Sabedoria');
  retorno['carisma'] = obter_valores_setados_atributos('carisma','Carisma');
  return retorno;
}
...
document.getElementById('classes').addEventListener('input',event=>{
  let classes_selecionada = document.getElementById('classes').options[document.getElementById('classes').selectedIndex].value;

  if (COMBO_SELECIONADO == 'nenhum') {
    if (classes_selecionada != 'Todas') {
  ...
  if (COMBO_SELECIONADO == 'classes') {
    ...
    if (classes_selecionada != 'Todas') {
      CLASSES[classes_selecionada]["Raças Permitidas"].forEach((classes_selecionada, i) => {
        COMBO_RACAS.forEach((item, i) => {
          if (classes_selecionada == item.value) {
            criarOption(document.getElementById('racas'),item.value,item.texto);
          }
        });
      });
    } else {
...
document.getElementById('racas').addEventListener('input',event=>{
  ...
  if (COMBO_SELECIONADO == 'racas') {
    ...
    if (racas_selecionada != 'Todas') {
      ...
      keys_classes.forEach((key_classe, i) => {
        if (CLASSES[key_classe]["Raças Permitidas"].includes(racas_selecionada)) {
          criarOption(document.getElementById('classes'),COMBO_CLASSES[COMBO_CLASSES_INDEX[key_classe]].value,COMBO_CLASSES[COMBO_CLASSES_INDEX[key_classe]].texto);
        }
      });
    } else {
      COMBO_CLASSES.forEach((item, i) => {
        criarOption(document.getElementById('classes'),item.value,item.texto);
      });
    }
  }
});
